import 'package:test/test.dart';
import 'package:openapi/openapi.dart';

// tests for HealthcareProviderDetail
void main() {
  final instance = HealthcareProviderDetailBuilder();
  // TODO add properties to the builder and call build()

  group(HealthcareProviderDetail, () {
    // int locationId
    test('to test the property `locationId`', () async {
      // TODO
    });

    // int institutionId
    test('to test the property `institutionId`', () async {
      // TODO
    });

    // String title
    test('to test the property `title`', () async {
      // TODO
    });

    // String institutionType
    test('to test the property `institutionType`', () async {
      // TODO
    });

    // String street
    test('to test the property `street`', () async {
      // TODO
    });

    // String houseNumber
    test('to test the property `houseNumber`', () async {
      // TODO
    });

    // String city
    test('to test the property `city`', () async {
      // TODO
    });

    // String postalCode
    test('to test the property `postalCode`', () async {
      // TODO
    });

    // String phoneNumber
    test('to test the property `phoneNumber`', () async {
      // TODO
    });

    // String fax
    test('to test the property `fax`', () async {
      // TODO
    });

    // String email
    test('to test the property `email`', () async {
      // TODO
    });

    // String website
    test('to test the property `website`', () async {
      // TODO
    });

    // String ico
    test('to test the property `ico`', () async {
      // TODO
    });

    // BuiltList<String> category
    test('to test the property `category`', () async {
      // TODO
    });

    // String specialization
    test('to test the property `specialization`', () async {
      // TODO
    });

    // String careForm
    test('to test the property `careForm`', () async {
      // TODO
    });

    // String careType
    test('to test the property `careType`', () async {
      // TODO
    });

    // String substitute
    test('to test the property `substitute`', () async {
      // TODO
    });

    // String lat
    test('to test the property `lat`', () async {
      // TODO
    });

    // String lng
    test('to test the property `lng`', () async {
      // TODO
    });

  });
}
